...
app.post('/send', checkBot, (req, res) => {
    ...
    posts.push({ id: Date.now(), real_name, display_name: isHidden ? 'Gizli Üye' : real_name, class: clazz, message: msg, approved: !isHidden, admin: false, date: new Date().toLocaleString('tr-TR') });
    savePosts(posts);
    
    if(isHidden) {
        res.send(layout(`<div class="container"><div class="card" style="text-align:center;"><h3 style="color:var(--gold)">Lütfen admin onayı bekleyin ⏳</h3><p>Gizli gönderiniz incelendikten sonra yayına alınacaktır.</p><a href="/posts" class="btn btn-blue">GERİ DÖN</a></div></div>`));
    } else {
        res.redirect('/posts');
    }
});
...
app.post('/admin/login', (req, res) => { 
    if(req.body.pw === 'sevemezsiniz') { 
        req.session.isAdmin = true; 
        res.redirect('/admin/panel'); 
    } else {
        res.send(layout(`<div class="container"><div class="card" style="text-align:center;"><h3 style="color:var(--red)">Yolun sonu şifre yanlış</h3><a href="/admin" class="btn btn-red">TEKRAR DENE</a></div></div>`));
    }
});
...
app.get('/admin/panel', (req, res) => {
    ...
    const pending = allPosts.filter(p => !p.approved && !p.admin);
    const approved = allPosts.filter(p => p.approved);

    let pendingHtml = pending.map(p => `<div class="card" style="border-left:4px solid var(--red)"><b>KİM: ${p.real_name} (Gizli)</b><p>${p.message}</p><a href="/admin/ok/${p.id}">ONAYLA</a> | <a href="/admin/del/${p.id}">SİL</a></div>`).join('');
    let approvedHtml = approved.map(p => `<div class="card" style="border-left:4px solid var(--green)"><b>PAYLAŞAN: ${p.real_name}</b><p>${p.message}</p><a href="/admin/del/${p.id}" style="color:var(--red)">YAYINDAN KALDIR/SİL</a></div>`).join('');
    
    res.send(layout(`<div class="container"><h3>Yönetim Paneli</h3><form action="/admin/announce" method="POST"><textarea name="m" placeholder="Resmi Duyuru..."></textarea><button class="btn btn-blue">PAYLAŞ</button></form><hr><h4>Bekleyen Onaylar</h4>${pendingHtml || 'Bekleyen yok.'}<hr><h4>Yayındaki Tüm Gönderiler</h4>${approvedHtml}</div>`));
});
